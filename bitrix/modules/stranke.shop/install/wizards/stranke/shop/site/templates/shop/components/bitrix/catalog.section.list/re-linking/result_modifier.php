...
foreach ($arResult['SECTIONS'] as $key => $arSection) {
    $arSectionIdList[$arSection['ID']] = $key;
}

$arFilter = [
    'IBLOCK_ID'=> $arParams['IBLOCK_ID'],
    'ID' => array_keys($arSectionIdList),
];
$arSelect = ['ID', 'UF_SEO_SECTION', 'UF_SEO_SECTIONS'];
$dbSections = CIBlockSection::GetList([], $arFilter, false, $arSelect);
while ($arSection = $dbSections->Fetch()) {
    if (!$arSection['UF_SEO_SECTION']) {
        unset($arResult['SECTIONS'][$arSectionIdList[$arSection['ID']]]);
    }
}
