...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        reachGoal: function (goal, section) {
            ...
            const counters = Object.keys(window).filter(k => /^yaCounter.*/.test(k));
            if (counters.length) {
                counter = counters[0].replace('yaCounter', '');
            }

            var useGoals = this.params.USE_YM_GOALS == 'Y' && typeof window['yaCounter' + counter] !== 'undefined';
            var goalId;
        ...
        editSection: function (section) {
            ...
            $('.bx-soa').css('min-height', $('#bx-soa-properties').css('height'));
        },
        ...
        createPaySystemItem: function (item) {
            ...
            if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
                title = BX.create('DIV', {props: {className: 'bx-soa-pp__item-title'}, text: item.NAME});
            }

            var description = BX.create('DIV', {
                props: {className: 'bx-soa-pp__item-description'},
                text: item.DESCRIPTION
            });

            info = BX.create('DIV', {
                props: {className: 'bx-soa-pp__item-info'},
                children: [title, description]
            });

            var inputItem = BX.create('INPUT', {
                props: {
                    id: 'ID_PAY_SYSTEM_ID_' + paySystemId,
                    name: 'PAY_SYSTEM_ID',
                    type: 'checkbox',
                    className: 'bx-soa-pp__item-input',
                    value: paySystemId,
                    checked: checked
                }
            });

            var inputCheckbox = BX.create('div', {
                props: {className: 'bx-soa-pp__item-checkbox'},
                children: [BX.create('SPAN')]
            });

            itemNode = BX.create('DIV', {
                props: {className: 'bx-soa-pp__item'},
                children: [inputItem, inputCheckbox, info],
                events: {
            ...
            if (checked)
                BX.addClass(itemNode, 'bx-soa-pp__item_selected');
        ...
        selectPaySystem: function (event) {
            ...
            var innerPsAction = BX.hasClass(target, 'bx-soa-pp-inner-ps') ? target : BX.findParent(target, {className: 'bx-soa-pp-inner-ps'}),
                actionSection = BX.hasClass(target, 'bx-soa-pp__item') ? target : BX.findParent(target, {className: 'bx-soa-pp__item'}),
            ...
            if (innerPsAction) {
                ...
                if (innerPaySystemCheckbox.checked) {
                    BX.removeClass(innerPaySystemSection, 'bx-soa-pp__item_selected');
                    innerPaySystemCheckbox.checked = false;
                }
                else {
                    BX.addClass(innerPaySystemSection, 'bx-soa-pp__item_selected');
                    innerPaySystemCheckbox.checked = true;
                }
            }
            else if (actionSection) {
                if (BX.hasClass(actionSection, 'bx-soa-pp__item_selected'))
                    return BX.PreventDefault(event);

                if (innerPaySystemCheckbox && innerPaySystemCheckbox.checked && fullPayFromInnerPaySystem) {
                    BX.addClass(actionSection, 'bx-soa-pp__item_selected');
                    ...
                    selectedSection = this.paySystemBlockNode.querySelector('.bx-soa-pp__item.bx-soa-pp__item_selected');
                    BX.addClass(actionSection, 'bx-soa-pp__item_selected');
                    actionInput = actionSection.querySelector('input[type=checkbox]');
                    actionInput.checked = true;

                    if (selectedSection) {
                        BX.removeClass(selectedSection, 'bx-soa-pp__item_selected');
        ...
        editActiveDeliveryBlock: function (activeNodeMode) {
            ...
            // console.info(this.deliveryHiddenBlockNode)
            if (this.initialized.delivery) {
                if (BX.firstChild(this.deliveryHiddenBlockNode)) {
                    BX.remove(BX.lastChild(node));
        ...
        createDeliveryItem: function (item) {
            var checked = item.CHECKED == 'Y',
                ...
                logotype, info, image, title, itemNode, logoNode;
            ...
            if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y') {
                title = BX.create('DIV', {
                    props: {className: 'bx-soa-pp__item-title'},
            ...
            labelNodes.push(description);

            var info_cost = false;
            if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined') {
                info_cost = BX.create('DIV', {
                    props: {className: 'bx-soa-pp__item-cost'},
                    html:
                        BX.message('STRANKE_SOA_DELIVERY_PRICE') + ' ' +
            ...
            info = BX.create('DIV', {
                props: {
                    className: 'bx-soa-pp__item-info'
            ...
            itemNode = BX.create('DIV', {
                props: {className: 'bx-soa-pp__item'},
                children: [input, checkbox, info, info_cost],
                events: {click: BX.proxy(this.selectDelivery, this)}
            });
            checked && BX.addClass(itemNode, 'bx-soa-pp__item_selected');
        ...
        selectDelivery: function (event) {
            ...
            var target = event.target || event.srcElement,
                actionSection = BX.hasClass(target, 'bx-soa-pp__item') ? target : BX.findParent(target, {className: 'bx-soa-pp__item'}),
                selectedSection = this.deliveryBlockNode.querySelector('.bx-soa-pp__item.bx-soa-pp__item_selected'),
        ...
        editPickUpList: function (isNew) {
            ...
            if (!recommendList || !recommendList.querySelector('.bx-soa-pp__item.bx-soa-pp__item_selected')) {
        ...
        createPickUpItem: function (currentStore, options) {
            ...
            var imgClassName = 'bx-soa-pp__item-info',
            ...
            var childrenItemAdress = [];

            var adress = BX.util.htmlspecialchars(currentStore.TITLE)
            childrenItemAdress.push(adress);
            if (options.distance) {
                var distance = ' ( ~' + options.distance + ' ' + BX.message('SOA_DISTANCE_KM') + ' ) ';
                childrenItemAdress.push(distance);
            }

            html = this.getStoreInfoHtml(currentStore);
            storeNode = BX.create('DIV', {
                props: {className: 'bx-soa-pp__item', id: 'store-' + currentStore.ID},
                children: [
                    BX.create('DIV', {
                        props: {className: 'bx-soa-pp__item-checkbox'},
                        children: [BX.create('SPAN')],
                    ...
                    BX.create('DIV', {
                        ...
                        children: [
                            BX.create('DIV', {props: {className: 'bx-soa-pp__item-title'}, text: currentStore.TITLE}),
                            logoNode,
            ...
            if (options.selected)
                BX.addClass(storeNode, 'bx-selected bx-soa-pp__item_selected');
        ...
        selectStore: function (event) {
            ...
            if (BX.type.isString(event)) {
                ...
                storeItem = BX.hasClass(target, 'bx-soa-pp__item')
                    ? target
                    : BX.findParent(target, {className: 'bx-soa-pp__item'});
            }

            if (storeItem && storeInput) {
                if (BX.hasClass(storeItem, 'bx-soa-pp__item_selected'))
                    return;

                selectedPickUp = this.pickUpBlockNode.querySelector('.bx-soa-pp__item_selected');
                storeItemId = storeItem.id.substr('store-'.length);

                BX.removeClass(selectedPickUp, 'bx-soa-pp__item_selected');
                BX.addClass(storeItem, 'bx-soa-pp__item_selected');
        ...
        getPropertyRowNode: function (property, propsItemsContainer, disabled) {
            ...
            if (!!settings.MAXLENGTH) {
                var width = parseInt(settings.MAXLENGTH);
                BX.style(propsItemNode, 'width', 'calc(' + width + '% - 10px)');
                BX.style(propsItemNode, 'float', 'left');
            }
            ...
            if (disabled) {
                ...
                if (!!settings.MAXLENGTH) {
                    BX.addClass(propsItemNode, "form-group_custom-size");
                }
        ...
        alterProperty: function (settings, propContainer) {
            ...
            if (textNode) {
                ...
                if (settings.IS_PHONE == 'Y') {
                    ...
                    textNode.setAttribute('is_phone', 'Y');
                    $(textNode).phone_mask();
        ...
        showValidationResult: function (inputs, errors) {
            ...
            var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
                ...
                container = formGroup.querySelector('.soa-property-container'),
            ...
            if (errors.length)
                this.showErrorTooltip(tooltipId, container, errors.join('<br>'));
            else
        ...
        editTotalBlock: function () {
            ...
            var lines = BX.create('div', {props: {className: 'bx-soa-cart-total-lines'}});
            this.totalInfoBlockNode.appendChild(lines);
            ...
            //lines.appendChild(this.createTotalUnit(BX.message('SOA_SUM_SUMMARY'), priceHtml, params));

            if (this.options.showOrderWeight) {
                lines.appendChild(this.createTotalUnit(BX.message('SOA_SUM_WEIGHT_SUM'), total.ORDER_WEIGHT_FORMATED));
            }
            ...
            // {
            // 	lines.appendChild(this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, params));
            // }
            ...
            if (this.options.showDiscountPrice) {
                ...
                lines.appendChild(this.createTotalUnit(discText, total.DISCOUNT_PRICE_FORMATED, {highlighted: true}));
            }

            if (this.options.showPayedFromInnerBudget) {
                lines.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED));
                lines.appendChild(this.createTotalUnit(BX.message('SOA_SUM_PAYED'), total.PAYED_FROM_ACCOUNT_FORMATED));
                lines.appendChild(this.createTotalUnit(BX.message('SOA_SUM_LEFT_TO_PAY'), total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED, {total: true}));
            }
            else {
                lines.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED, {total: true}));
            }

            if (parseFloat(total.PAY_SYSTEM_PRICE) >= 0 && this.result.DELIVERY.length) {
                lines.appendChild(this.createTotalUnit(BX.message('SOA_PAYSYSTEM_PRICE'), '~' + total.PAY_SYSTEM_PRICE_FORMATTED));
            }
                    ...
                    BX.create('DIV', {
                        ...
                        children: [
                            BX.create('A', {
                                props: {
                                    href: 'javascript:void(0)',
                                    className: 'btn bg_main btn-order-save'
                                },
        ...
        setAnalyticsDataLayer: function (action, id) {
            ...
            switch (action) {
                ...
                case 'purchase':
                    ...
                    window.dispatchEvent(new Event('purchase'));
                    break;
            }
