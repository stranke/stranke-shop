...
//$this->addExternalCss($templateFolder . '/css/catalog-element.css');

if (isset($_POST['ajax_basket']) && $_POST['ajax_basket'] == "Y") {
    $APPLICATION->RestartBuffer();
    $item_id = $_POST['add_item'];
    $answer = array();
    if (!Add2BasketByProductID($item_id)) {
        $answer['STATUS'] = "ERR";
        if ($ex = $APPLICATION->GetException())
            $answer['ERROR'] = $ex->GetString();
        else
            $answer['ERROR'] = GetMessage("CATALOG_ERROR2BASKET");
    } else {
        $answer['STATUS'] = "OK";
        ob_start();
        include($_SERVER["DOCUMENT_ROOT"] . SITE_TEMPLATE_PATH . '/includes/header/header_cart.php');
        $answer['CART'] = ob_get_contents();
        ob_end_clean();
    }
...
//  'OFFERS_SORT_ORDER' => $arParams['OFFERS_SORT_ORDER'],
    ...
    'ajax_basket' => $_POST['ajax_basket'],
    'add_item' => $_POST['add_item'],
    'TYPE' => $arResult['VARIABLES']['ELEMENT_TAB'] ? $arResult['VARIABLES']['ELEMENT_TAB'] : 'description',
);
?>
<div class="product_detail">
    ...
    $componentElementParams['ADD_SECTIONS_CHAIN'] = 'N';
    $componentElementParams['ADD_ELEMENT_CHAIN'] = 'N';
...
while ($arPropValue = $dbPropValues->Fetch()) {
    ...
    $GLOBALS['FILTER_RELATED_PRODUCTS']['ID'][] = $arPropValue['VALUE'];
}
?>
    ...
    <? $APPLICATION->IncludeComponent(
        ...
        array(
            ...
            "SECTION_USER_FIELDS" => array(
                0 => "",
                1 => "UF_DETAIL_PAGE_URL",
                2 => "",
            ),
            "INCLUDE_SUBSECTIONS" => "Y",
            "SPECIAL_TITLE" => "Хиты продаж",
            "SHOW_ALL_WO_SECTION" => "Y",
            "COMPONENT_TEMPLATE" => "bestsellers",
            "SECTION_ID" => "",
            "SECTION_CODE" => "",
            "FILTER_NAME" => "FILTER_RELATED_PRODUCTS",
            "CUSTOM_FILTER" => "",
            ...
            "TITLE" => GetMessage('TITLE_RELATED_PRODUCTS'),
        ),
        false
    ); ?>
<? endif */ ?>
