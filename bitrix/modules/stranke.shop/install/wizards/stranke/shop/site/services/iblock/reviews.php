...
$iblockType = "content";
$iblockCode = "content_reviews_" . WIZARD_SITE_ID;
$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH . "/xml/" . LANGUAGE_ID . "/reviews/reviews.xml";
...
$arFilter = ['IBLOCK_ID' => $iblockID, 'CODE' => 'common'];
$dbSections = CIBlockSection::GetList([], $arFilter);
while ($arSection = $dbSections->GetNext()) {
    switch($arSection['CODE']) {
        case 'common':
            $commonSection = $arSection; break;
        case 'products':
            $commonSection = $productsSection; break;
    }
}

foreach (['/_index.php', '/otzyvy/index.php'] as $file) {
    CWizardUtil::ReplaceMacros(
        WIZARD_SITE_PATH . $file, [
            "REVIEWS_IBLOCK_ID" => $iblockID,
            "REVIEWS_COMMON_SECTION_ID" => $commonSection['ID'],
        ]
    );
}
