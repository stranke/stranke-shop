...
foreach ($this->basketItems as $row)
{
    ...
    // DETAIL_PICTURE
    // PREVIEW_PICTURE_SRC
    // PREVIEW_PICTURE_SRC_2X
    // PREVIEW_PICTURE_SRC_ORIGINAL
    // DETAIL_PICTURE_SRC
    // DETAIL_PICTURE_SRC_2X
    // DETAIL_PICTURE_SRC_ORIGINAL

    $arPictureSize = ['width' => 272, 'height' => 167];
    $previewPicture = CFile::ResizeImageGet($row['PREVIEW_PICTURE'], $arPictureSize, 2);
    $detailPicture = CFile::ResizeImageGet($row['DETAIL_PICTURE'], $arPictureSize, 2);

    $row['PREVIEW_PICTURE_SRC'] = $previewPicture['src'];
    $row['DETAIL_PICTURE_SRC'] = $detailPicture['src'];
