...
function onClickCardHeader() {
	...
	const $cardBody = $cardHeader.siblings('.bx-basket__card-body');
	const $card = $cardHeader.parent();

	if ($card.hasClass('bx-basket__card_opened')) {
		$cardBody.slideUp();
		$card.removeClass('bx-basket__card_opened');
	} else {
		$cardBody.slideDown();
		$card.addClass('bx-basket__card_opened');
	}
}

$(function() {
	const $cardHeader = $('.bx-basket__card-header');
	$cardHeader.on('click', onClickCardHeader)
});
