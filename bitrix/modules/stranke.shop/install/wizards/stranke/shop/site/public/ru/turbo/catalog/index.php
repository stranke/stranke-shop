...
if (!isset($XML_DATA) || !is_array($XML_DATA))
    $arRunErrors[] = 'YANDEX_ERR_BAD_XML_DATA';
...
$IBLOCK_ID = (int)$app->config->iblockIdCatalog;
...
if (empty($site))
{
    $arRunErrors[] = 'BX_CATALOG_EXPORT_YANDEX_ERR_BAD_SITE';
}
else
{
...
else
{
    ...
    if (!empty($arOffers['IBLOCK_ID']))
    {
        ...
        {
            $arRunErrors[] = 'YANDEX_ERR_BAD_OFFERS_IBLOCK_ID';
        }
    ...
    if ($boolOffers)
    {
        ...
        {
            $arRunErrors[] = 'YANDEX_ERR_SKU_SETTINGS_ABSENT';
        }
        else
        {
            ...
            {
                $arRunErrors[] = 'YANDEX_SKU_EXPORT_ERR_CONDITION_ABSENT';
            }
            if (YANDEX_SKU_EXPORT_PROP == $arSKUExport['SKU_EXPORT_COND'])
            {
                ...
                {
                    $arRunErrors[] = 'YANDEX_SKU_EXPORT_ERR_PROPERTY_ABSENT';
                }
                else
                {
                    ...
                    {
                        $arRunErrors[] = 'YANDEX_SKU_EXPORT_ERR_PROPERTY_COND_ABSENT';
                    }
                    else
                    {
                        if ($arSKUExport['SKU_PROP_COND']['COND'] == 'EQUAL' || $arSKUExport['SKU_PROP_COND']['COND'] == 'NONEQUAL')
                        {
                            ...
                            {
                                $arRunErrors[] = 'YANDEX_SKU_EXPORT_ERR_PROPERTY_VALUES_ABSENT';
                            }
                        }
                    }
                }
...
if (empty($arRunErrors))
{
    ...
    {
        $arRunErrors[] = 'YANDEX_ERR_NO_SECTION_LIST';
    }
}
...
$turboPage = '';
$turboPage .= '<?xml version="1.0" encoding="windows-1251"?>' . "\n";
$turboPage .= '<!DOCTYPE yml_catalog SYSTEM "shops.dtd">' . "\n";
$turboPage .= '<yml_catalog date="'.date("Y-m-d H:i").'">' . "\n";
$turboPage .= '<shop>' . "\n";
$turboPage .= '<name>'.$APPLICATION->ConvertCharset(htmlspecialcharsbx($site['SITE_NAME']), LANG_CHARSET, 'windows-1251')."</name>\n";
$turboPage .= '<company>'.$APPLICATION->ConvertCharset(htmlspecialcharsbx($site['COMPANY_NAME']), LANG_CHARSET, 'windows-1251')."</company>\n";
$turboPage .= '<url>'.$usedProtocol.htmlspecialcharsbx($site['SERVER_NAME'])."</url>\n";
$turboPage .= '<platform>1C-Bitrix</platform>'."\n";
...
if (empty($arRunErrors))
{
    if ($boolNeedRootSection) {
        $turboPage .= '<category id="'.$intMaxSectionID.'">'.yandex_text2xml(GetMessage('YANDEX_ROOT_DIRECTORY'), true).'</category>'."\n";
    }
