...
foreach ($arResult["CATEGORIES"] as $category_id => $arCategory) {
    foreach ($arCategory["ITEMS"] as $i => $arItem) {
        if (isset($arItem["ITEM_ID"])) {
            ...
            if (
                $arItem["MODULE_ID"] == "iblock" && array_key_exists($arItem["PARAM2"], $arCatalogs) && substr($arItem["ITEM_ID"], 0, 1) !== "S"
            ) {
...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock")) {
    ...
    while ($arElm = $rsElements->GetNextElement()) {
        $arElement = $arElm->GetFields();
        $arElementProps = $arElm->GetProperties();
        ...
        if (!$arElement["PRICES"]) {
            $SKU_Test = CCatalogSKU::getExistOffers($arElement["ID"]);
            if ($SKU_Test[$arElement["ID"]]) {
                $arElement["PRICES"] = '<span class="no_color">От</span> ' . CurrencyFormat($arElementProps["SORTING_PRICE"]["VALUE"], 'RUB');
            } else {
                $arElement["PRICES"] = CurrencyFormat($arElementProps["SORTING_PRICE"]["VALUE"], 'RUB');
            }
        } else {
            $arElement["PRICES"] = CurrencyFormat($arElementProps["SORTING_PRICE"]["VALUE"], 'RUB');
        }
            ...
            if (array_key_exists($arItem["ITEM_ID"], $arResult["ELEMENTS"])) {
                ...
                if ($arParams["SHOW_PREVIEW"] == "Y") {
                    if ($arElement["PREVIEW_PICTURE"] > 0) {
                        ...
                        $res = CIBlockElement::GetByID(20);
                        if ($ar_res = $res->GetNext()) {
                            $arFile = CFile::GetFileArray($ar_res["DETAIL_PICTURE"]);
                            $arElement["PICTURE"] = CFile::ResizeImageGet($arFile, array("width" => $PREVIEW_WIDTH, "height" => $PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL, true);
                        }
                    }
                }
            }
...
$arResult['ANIMATE_REPLACE'] = array();
$arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM");
$arFilter = Array("IBLOCK_CODE" => ('catalog_products_' . SITE_ID), "ACTIVE" => "Y");
$res = CIBlockElement::GetList(Array("RAND" => "ASC"), $arFilter, false, Array("nPageSize" => 50), $arSelect);
while ($arFields = $res->GetNext()) {
    $arResult['ANIMATE_REPLACE_PRODUCT'] = explode(' ', $arFields['~NAME'])[0];
    $arResult['ANIMATE_REPLACE'][] = mb_substr($arFields['~NAME'], 0, 30);
}
