...
function delLOGO(elem) {
    var data = {'id': elem};
    $.ajax({
        url: '/ajax/forms/deletefileid.php',
        type: 'POST',
        data: data,
        dataType: 'json',
        success: function (data) {
            ...
            console.info($('input[name="' + data.id + '"]').parent().parent().children('.settings__field-img').children('img').attr('src', data.file))
...
$(document).ready(function () {
    $('.pcr-button').click(function () { // задаем функцию при нажатиии на элемент <button>
            var nameattr = $(this).parent('.pickr').parent('.settings__field-control').children('input').attr('name')
            var coloraction = $(this).attr('style')
            if (nameattr == 'PROP[COLOR_BACKGROUND_ACTION]') {
                var coloraction = coloraction.replace('color', 'background');

                $('.price-reduction').attr('style', coloraction)
            }
            if (nameattr == 'PROP[COLOR_TEXT_ACTION]') {
                $('.price-reduction__text').attr('style', coloraction)
            }
        }
    )
});